...
import (
	...
	"github.com/ntphuy2001/golang-jwt-project/database"
	helper "github.com/ntphuy2001/golang-jwt-project/helpers"
	"github.com/ntphuy2001/golang-jwt-project/models"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if countEmail|countPhone > 0 {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "this email or phone number are occupied"})
			return
		}
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token, refreshToken, _ := helper.GenerateAllTokens(*foundedUser.Email, *foundedUser.FirstName, *foundedUser.LastName, *foundedUser.UserType, *&foundedUser.UserId)
		helper.UpdateAllTokens(token, refreshToken, foundedUser.UserId)
		userCollection.FindOne(ctx, bson.M{"user_id": foundedUser.UserId}).Decode(&foundedUser)
...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		projectStage := bson.D{{"$project", bson.D{
			...
			{"total_count", 1},
			{"user_items", bson.D{
				{"$slice", []interface{}{"$date", startIndex, recordPerPage}},
			}},
		}}}
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occurred while get users"})
			return
		}
		var allUsers []bson.M
